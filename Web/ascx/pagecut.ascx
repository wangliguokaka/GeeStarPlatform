<%@ Control Language="C#" AutoEventWireup="true" CodeFile="pagecut.ascx.cs" Inherits="RGPWEB.ascx.pagecut" %>

<script src="/jQuery/SplitPage.js" type="text/javascript"></script>

<div class="dPage" style="">
    <div style="height: 15px; line-height: 15px;">
        <div style="float: left;" id="fybox">
            <%--共 <span id="sTotalRecords"><%=iRowAllCount%></span>条--%>
            <%= GetGlobalResourceObject("Resource","PageNumber").ToString() %>：<span id="sPage"><%=iPageIndex == 0 || bPageClear ? 1: iPageIndex %>/<%=iPageNum %></span><%= GetGlobalResourceObject("Resource","Page").ToString() %>
            <%
                string firstdisabled = "";
                string lastdisabled = "";
                string godisabled = "";
                if (iPageIndex == iPageNum || iPageNum == 0)
                {
                    lastdisabled = "style=\"text-decoration:none; color:#999999; cursor:default\"";
                }
                if (iPageIndex == 1)
                {
                    firstdisabled = "style=\"text-decoration:none; color:#999999; cursor:default\"";
                }
            %>
            <a href="javascript:void(0)" <%=firstdisabled %> onclick="btnFirst_click(); return false;">
                <%= GetGlobalResourceObject("Resource","FirstPage").ToString() %></a> <a href="javascript:void(0)" <%=firstdisabled %> onclick="btnPre_click(); return false;">
                    <%= GetGlobalResourceObject("Resource","PreviousPage").ToString() %></a> <a href="javascript:void(0)" <%=lastdisabled %> onclick="btnNext_click(); return false;">
                        <%= GetGlobalResourceObject("Resource","NextPage").ToString() %></a> <a href="javascript:void(0)" <%=lastdisabled %> onclick="btnLast_click(); return false;">
                            <%= GetGlobalResourceObject("Resource","LastPage").ToString() %></a> <%= GetGlobalResourceObject("Resource","GoTo").ToString() %>： </div>
        <div style="float: left;" >
           <input id="txtGo" name="txtGo" style="width: 24px; height: 13px; border: 1px solid #444444;"
                type="text" /></div><div style="float: left;padding-left:10px;">
            <input class="button" type="button" style="width: 30px; height: 18px; border: 1px solid #444444;
                font-size: 10px" name="Submit" <%=godisabled %> onclick="btnGo_click(); return false;"
                value="GO" /></div>
        
    </div>
    <input id="hPageID" name="hPageID" type="hidden" value="<%=iPageIndex==0 || bPageClear?1:iPageIndex%>"
        style="width: 1px" />
    <input id="hPageNum" name="hPageNum" type="hidden" value="<%=iPageNum %>" style="width: 1px" />
</div>
