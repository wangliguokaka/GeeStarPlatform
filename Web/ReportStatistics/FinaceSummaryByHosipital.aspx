<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FinaceSummaryByHosipital.aspx.cs" 
    MasterPageFile="~/ReportStatistics/ReportStatistics.master" Inherits="ReportStatistics_FinaceSummaryByHosipital" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <script type="text/javascript">
        function addHospital() {
            ...
            $.ajax({
                url: "SelectHospital.aspx?HospitalList=" + $("#HospitalList").val() + "&radio=1&seller=" + seller,
        ...
        function SearchInfoList() {
            if ($("#HospitalList").val() == "")
            {
                alertdialog('<%= GetGlobalResourceObject("SystemResource","PleaseSelectHospital").ToString() %>！');
                return false;
            }
    ...
    <div class="nowposition">
       ...
       <b><%= GetGlobalResourceObject("SystemResource","SettleAccounts2").ToString() %>&nbsp;</b></div> 
    <div style="height:90%;">
        <div class="searchbox1">
            ...
            <a class="abtn01" href="javascript:void(0)"><%= GetGlobalResourceObject("SystemResource","Payway").ToString() %>：</a><asp:CheckBoxList runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow"  ID="charges" DataTextField="DictName" DataValueField="Code" ></asp:CheckBoxList>&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="searchbox1"><%= GetGlobalResourceObject("SystemResource","DateToFactory").ToString() %>
        <input type="radio" name="dateselect" id="indate" value="0" <%if(String.IsNullOrEmpty(Request["dateselect"]) || Request["dateselect"] == "0") {%>checked="checked"<%} %> /> &nbsp;&nbsp;<%= GetGlobalResourceObject("SystemResource","OutFacotry").ToString() %><input type="radio" name="dateselect" id="outdate" value="1" <%if(Request["dateselect"] == "1") {%>checked="checked"<%} %> />&nbsp;&nbsp;<input id="txtdatefrom" readonly="readonly" value="<%=Request["txtdatefrom"] %>" onfocus="WdatePicker({el:'txtdatefrom',dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="txtdatefrom" style="width: 140px" type="text" />
                    ...
                    <img onclick="WdatePicker({el:'txtpredate',dateFmt:'yyyy-MM-dd'})" src="../My97DatePicker/skin/datePicker.gif"style="width: 16px; height: 22px;" align="absmiddle"/>
        </div>
        <div class="searchbox1"><%= GetGlobalResourceObject("SystemResource","Barcode").ToString() %>:
             <asp:TextBox runat="server" ID="OrderNo"></asp:TextBox> &nbsp;&nbsp;&nbsp;&nbsp;<a onclick="SearchInfoList()" href="javascript:void(0)" class="btn02"><%= GetGlobalResourceObject("SystemResource","Search").ToString() %></a>
        </div>
        <div style="height:80%;">
            <rsweb:ReportViewer ID="FinanceReportViewer" runat="server" Font-Names="Verdana" Font-Size="8pt" CssClass="reportView"  Height="75%"  WaitMessageFont-Names="Verdana" WaitMessageFont-Size="14pt">
                <LocalReport ReportPath="ReportStatistics\Rdlc\FinaceSummaryByHosipital.rdlc"></LocalReport>
            </rsweb:ReportViewer>
        </div>
