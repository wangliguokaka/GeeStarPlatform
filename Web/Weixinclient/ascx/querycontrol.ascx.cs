...
namespace Weixin.RGPWEB.ascx
{
    ...
    public partial class querycontrol : System.Web.UI.UserControl
    {
        ...
        public ConditionComponent GetCondtion()
        {
            ...
            string datetype = (Request["datetype"]==null && !String.IsNullOrEmpty(DispalyType))?"0": Request["datetype"];
            string datefrom = Request["datefrom"];
            string dateto = Request["dateto"];
            string seller = String.IsNullOrEmpty(Request["seller"])? userSellerID: Request["seller"];
            string hospital = String.IsNullOrEmpty(Request["hospital"]) ? userHospitalID : Request["hospital"];
            string doctor = String.IsNullOrEmpty(Request["doctor"]) ? userDoctorID : Request["doctor"];
            ...
            if (!string.IsNullOrEmpty(smallClass))
            {
                if (!smallClass.Equals("-1"))
                {
                    ...
                    if (String.IsNullOrEmpty(DispalyType))
                    {                       
                       ccWhere.AddComponent("orders.Order_ID", "(SELECT [Order_ID] FROM [OrdersDetail] where ProductId in (select id from products where SmallClass = N'" + smallClass + "'))", SearchComponent.In, SearchPad.And);
                    }
                    else
                    {
                        ccWhere.AddComponent("ordersdetail.ProductID", "(select id from products where SmallClass = N'" + smallClass + "')", SearchComponent.In, SearchPad.And);
            ...
            {
                if (!productLine.Equals("-1"))
                {
                    ccWhere.AddComponent("orders.Order_ID", "(SELECT [Order_ID] FROM [OrdersDetail] where productLine  = N'" + productLine + "')", SearchComponent.In, SearchPad.And);
                }
            }
            ...
            if (!string.IsNullOrEmpty(seller))
            {
                if (!seller.Equals("-1"))
                {
                    ccWhere.AddComponent("sellerid", seller, SearchComponent.Equals, SearchPad.And);
                }
            }
            if (!string.IsNullOrEmpty(hospital))
            {
                ...
                {
                    ccWhere.AddComponent("Hospitalid", hospital, SearchComponent.Equals, SearchPad.And);
                }
            }
            if (!string.IsNullOrEmpty(doctor))
            {
                if (!doctor.Equals("-1"))
                {                   
                    ccWhere.AddComponent("doctor", "(select top 1 doctor from doctor where id = " + doctor + ")", SearchComponent.In, SearchPad.And);

                }
            }
            if (!string.IsNullOrEmpty(patient))
            {
                ccWhere.AddComponent("patient", "%" + patient + "%", SearchComponent.Like, SearchPad.And);
            }
