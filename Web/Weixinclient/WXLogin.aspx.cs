...
public partial class Weixinclient_WXLogin : PageBase
{
    ...
    protected void CheckLogin()
    {
        ...
        if (!string.IsNullOrEmpty(txtUserName))
        {
            try
            {
                ...
                {
                    Page.ClientScript.RegisterStartupScript(GetType(), "sucess", "art.dialog({title: false,time: 2,icon: 'error',content: '同步脚本失败'});", true);
